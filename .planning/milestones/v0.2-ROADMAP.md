...
**Depends on:** None (foundation for remaining phases)
...
**Depends on:** Phase 5 (architecture detection patterns may influence guard paths)
...
**Depends on:** None (independent policy)
**Requirements:** LTS-01
**Plans:** 1 plan
...
**Key Decisions:**
- Use YAML folding (>-) for long repo lines to meet 160-char lint limit (05-01)
- Guard on first binary (gofumpt for Go, ruff for Python) since all tools installed together (06-01)
- Same path for both OS (~/.local/bin for uv) so guard path identical (06-01)
- Use ansible.builtin.command for ssh-keygen -F (no shell features needed) (07-01)
- Use deb822_repository over apt_repository for modern .sources format (07-02)
- LTS > stable > latest version hierarchy (08-01)

**Issues Resolved:**
- ARM64 architecture hardcoding preventing Raspberry Pi deployment
- False "changed" status on idempotent re-runs
- SSH known_hosts duplicate entries
- Fragile sed manipulation of Debian sources.list

**Issues Deferred:**
- CI/CD pipeline (deferred — personal repo tested on real machines)
- Molecule tests (deferred — real machine testing more accurate)
