...
Then update the "Add Docker repository (Debian)" task at line 50 to use dynamic arch:
- Change: `arch=amd64`
- To: `arch={{ deb_arch }}`
...
1. macOS task (line 14): Change URL from hardcoded `darwin-arm64` to dynamic:
   ```yaml
   url: "https://go.dev/dl/go{{ go_version }}.{{ go_os }}-{{ go_arch }}.pkg"
   ```

2. Linux task (line 29): Change URL from hardcoded `linux-amd64` to dynamic:
   ```yaml
