...
1. Update the "Add 1Password apt repository (Debian)" task (line 19):
   Add architecture check to the when clause:
   ```yaml
   when:
...
2. Update the "Install 1Password via apt (Debian)" task (line 33):
   Add architecture check to the when clause:
   ```yaml
   when:
     - ansible_facts['os_family'] == "Debian"
...
1. Update the "Add Microsoft Edge apt repository (Debian)" task (line 19):
   Add architecture check to the when clause:
   ```yaml
   when:
     - ansible_facts['os_family'] == "Debian"
...
2. Update the "Install Microsoft Edge via apt (Debian)" task (line 29):
   Add architecture check to the when clause:
   ```yaml
   when:
     - ansible_facts['os_family'] == "Debian"
