...
CCT() {
	...
	*)
		echo "CCT: not inside a /tmp/cct.* dir ($cur)" >&2
		return 1
		;;
	esac
...
_ccw_self_destruct() {
	...
	cur_toplevel=$(git rev-parse --show-toplevel 2>/dev/null) || {
		echo "CCW/CCS: not in a git repo" >&2
		return 1
	}
	main_root=$(_ccw_repo_root) || {
		echo "CCW/CCS: not in a git repo" >&2
		return 1
	}
	...
	"$wt_root"/*) ;;
	*)
		echo "CCW/CCS: not inside a ccw worktree ($cur_toplevel)" >&2
		return 1
		;;
	esac
