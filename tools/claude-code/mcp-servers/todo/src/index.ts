...
// Todo interface - compatible with Claude's built-in TodoWrite format
interface Todo {
  id: string;
  content: string;
  status: "pending" | "in_progress" | "completed";
  activeForm: string;
  // Extensions for multi-project/agent support
  project: string;
  assignee?: string;
...
class TodoManager {
  ...
  async syncTodos(
    ...
    // Add new todos
    const newTodos: Todo[] = todos.map((t, i) => ({
  ...
  async clearCompleted(project?: string): Promise<number> {
    const todos = await this.loadTodos();
    const initialLength = todos.length;
    const remaining = todos.filter((t) => {
      if (t.status !== "completed") return true;
      if (project && t.project !== project) return true;
      return false;
    });
...
server.tool(
  "add_todo",
  "Add a new todo item (compatible with Claude's TodoWrite format)",
  {
...
server.tool(
  "sync_todos",
  "Sync todos from Claude's built-in TodoWrite format. Replaces all todos for the project.",
  {
...
server.tool(
  "list_todos",
  "List todos, optionally filtered by status, priority, project, or assignee",
  {
    status: z
...
server.tool(
  "list_projects",
  "List all projects that have todos",
  {},
  async () => {
...
server.tool(
  "clear_completed",
  "Remove all completed todos, optionally for a specific project",
  {
