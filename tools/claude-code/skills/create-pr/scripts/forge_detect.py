...
- If the host appears in $CREATE_PR_GITEA_HOSTS (comma-separated) or
  $CREATE_PR_FORGEJO_HOSTS → gitea (CLI: tea)
- Override via $CREATE_PR_FORGE=github|gitlab|gitea (highest priority)
...
class ForgeInfo:
    ...
    cli_installed: bool
    cli_path: str | None
    override: bool  # True if $CREATE_PR_FORGE was used
...
def parse_remote_url(url: str) -> tuple[str, str, str]:
    ...
    if m and "@" in url.split("://", 1)[-1] and ":" in url.split("@", 1)[-1]:
        ...
        # https/http/git protocol: proto://[user@]host[:port]/owner/repo(.git)?
        m = re.match(
    ...
    # GitLab supports subgroups: owner/subgroup/repo. Keep everything before
    # the last slash as "owner" (project namespace); last segment is the repo.
    owner = "/".join(parts[:-1])
    repo = parts[-1]
...
def detect_forge_from_host(host: str) -> str:
    override = os.environ.get("CREATE_PR_FORGE", "").strip().lower()
    ...
    if host_l == "gitlab.com" or "gitlab" in host_l:
        return "gitlab"
    gitea_hosts = _split_env("CREATE_PR_GITEA_HOSTS")
    forgejo_hosts = _split_env("CREATE_PR_FORGEJO_HOSTS")
...
def detect(cwd: str | None, remote: str) -> ForgeInfo:
    ...
    return ForgeInfo(
        ...
        cli_path=cli_path,
        override=bool(os.environ.get("CREATE_PR_FORGE")),
    )
